...
USAGE() {
  ...
  echo -e "\033[34m 更新 template-snmpv2 配置中 SNMP V3 部分\033[0m"
  echo -e "\033[34m 更新 template-snmpv2 配置中 templates name 部分\033[0m"
}
...
sed -e "s#GROUP_NAME#${GROUP_NAME}#g" -e "s#IP_ADDR#${IP_ADDR}#g" -e "s#HOST_NAME#${HOST_NAME}#g" ${CWD}/template-snmpv2 | tee -a ${EXP_FILE} &> /dev/null
