...
USAGE() {
  echo "Usage:"
  echo "    $0 "
  echo -e "\033[33m 手动编辑 snmp_list 文件，填写“组名|主机名|IP地址”信息。请参考入下示例：\033[0m"
  echo -e "\033[34m vi /root/servers/snmp_list \033[0m"
  echo -e "\033[34m group1|webserver1|192.168.1.101 \033[0m"
  echo -e "\033[34m group1|webserver2|192.168.1.102 \033[0m"
  echo -e "\033[34m 更新 template-snmpv3 配置中 SNMP V3 部分\033[0m"
  echo -e "\033[34m 更新 template-snmpv3 配置中 templates name 部分\033[0m"
}
...
sed -e "s#GROUP_NAME#${GROUP_NAME}#g" -e "s#IP_ADDR#${IP_ADDR}#g" -e "s#HOST_NAME#${HOST_NAME}#g" ${CWD}/template-snmpv3 | tee -a ${EXP_FILE} &> /dev/null
